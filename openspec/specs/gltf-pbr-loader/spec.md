# gltf-pbr-loader Specification

## Purpose

Define the vendoring, parsing, and data-access contract that turns `infra::GLTFLoader` into a real cgltf-backed glTF 2.0 PBR loader for the renderer's asset pipeline. This capability specifies where the `cgltf` dependency lives in the tree, how ASCII `.gltf` files are parsed into geometry plus PBR material metadata, how texture URIs are surfaced to callers, and how failure modes are reported — so downstream systems (material instances, texture registry, integration tests) can depend on a stable, offline-buildable loader instead of the previous stub.
