...
The GUI system SHALL initialize Dear ImGui with the Vulkan rendering backend and the platform backend selected by the active window system. `Gui::InitParams` SHALL be:
...
4. Initialize the matching platform backend:
   - SDL builds: `ImGui_ImplSDL3_InitForVulkan(static_cast<SDL_Window*>(params.nativeWindowHandle))`
   - GLFW builds: `ImGui_ImplGlfw_InitForVulkan(static_cast<GLFWwindow*>(params.nativeWindowHandle), true)`
...
- **THEN** ImGui context SHALL be created, the active window backend and Vulkan backend SHALL both be initialized, and a Gui-owned descriptor pool SHALL be live
...
1. Start the active platform backend frame (`ImGui_ImplSDL3_NewFrame()` or `ImGui_ImplGlfw_NewFrame()`)
...
- **THEN** the active platform backend `NewFrame`, Vulkan `NewFrame`, and `ImGui::NewFrame` SHALL have been called in that order
...
### Requirement: GUI swapchain refresh

The GUI system SHALL refresh Dear ImGui's Vulkan image-count state after swapchain recreation.

#### Scenario: Swapchain image count changes

- **WHEN** the renderer recreates the swapchain with a different image count after `Gui::init()`
- **THEN** the GUI system SHALL call `ImGui_ImplVulkan_SetMinImageCount(newCount)` before the next ImGui frame is rendered
...
2. Shut down the active platform backend (`ImGui_ImplSDL3_Shutdown()` or `ImGui_ImplGlfw_Shutdown()`)
...
- **THEN** `ImGui_ImplVulkan_Shutdown()`, the active platform backend shutdown, descriptor pool destruction, and `ImGui::DestroyContext()` SHALL all occur before the method returns; `isInitialized()` SHALL return `false` afterwards
