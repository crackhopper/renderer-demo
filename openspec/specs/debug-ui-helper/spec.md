# debug-ui-helper Specification

## Purpose

Define a thin, ImGui-backed helper module that lives inside the `LX_Infra` library and provides reusable widget primitives (vector/scalar drags and sliders, labelled display rows, panel and section containers) together with a small set of domain-aware panels (frame stats, camera, directional light) for debug UI built on top of the existing ImGui overlay. This capability specifies the module location, namespace boundaries, public API surface, behavioral guarantees, and integration smoke test so that debug UIs can be assembled without leaking ImGui types into core code or inventing bespoke abstractions around scene primitives.
