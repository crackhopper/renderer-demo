...
The system SHALL define a YAML-based material asset file format (`.mat.yaml`). The format SHALL support:

- `shader`: required string identifying the shader family name
...
- **WHEN** a yaml file declares shader, global parameters, global resources, and per-pass overrides
...
The system SHALL provide a `loadGenericMaterial(yamlPath)` function that:

1. Parses the YAML material asset file
2. Resolves and compiles the shader for each configured pass with merged variants
