...
renderable 自带的 push constant 资源包装。底层缓冲 128 字节，但当前 ABI 只要求 `PC_Base` / `PC_Draw` 的 `model` 字段。
...
engine-wide draw push constant ABI。`PC_Draw` 现在只是 `PC_Base` 的别名，两者都只包含 `model`。
...
骨骼动画资源及其 GPU UBO。现在只提供 runtime 数据，不再提供 pipeline identity token。
